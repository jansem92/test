...
function MainRoute(props) {
  ...
  const fetchKeycloak = (refreshToken) =>
    new Promise((resolve, reject) => {
      const path = `/v1/user/update-token`
        HttpRequest.post({url: REACT_APP_AMWS + path, data: null, config: {
                headers: {
  ...
  return <PageContent pathName={propsExtra.pathName} propsExtra={propsExtra} />
}
